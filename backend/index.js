...
app.put("/update/:id", (req, res) => {
  ...
  const Payment = req.body.Payment;
  const sqlput =
    "UPDATE user SET name = ? , number = ? , dob = ? , services = ? , therapist = ? , payment = ? WHERE id = ?";
  db.query(
    sqlput,
    [name, number, dob, services, therapist, Payment, id],
    (error, result) => {
      if (error) {
        console.log(error);
      }
    ...
    if (result.length === 1) {
      bcrypt.compare(password, result[0].password, (error, response) => {
        if (response) {
          ...
          var token = jwt.sign({ user: result , pass:password }, "secretkey");
        //   var enter = user;
        //   var pass = password;
		console.log(result);
...
app.put('/LogUpdate' , (req,res)=>{
 const {fourth , third}= req.body;
 const sql='SELECT * FROM userid WHERE userid=? ;'
 db.query(sql , fourth , (err , result)=>{
   if(err){
     console.log(err);
   }
   console.log('your result for user is :' , result , result.length);
   bcrypt.hash(third, saltround, (error, hash) => {
    if (err) {
      console.log(error);
    }
    if(result.length) {
     const skl = 'UPDATE userid set userid=? , password=? WHERE userid=?';
     db.query(skl , [fourth , hash , fourth] , (error , resolt)=>{
       if(error)
       {
         console.log(error);
       }
       console.log("updated succeessfuullyy : " , resolt );
     } );
    }
   })
 } )
